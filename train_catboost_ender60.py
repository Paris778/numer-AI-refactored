"""Train the capital-path CatBoost Ender-60 model and record the run."""
...
def main() -> int:
    cfg = load_config("configs/catboost-ender60-fast.yaml")
    ...
    print("=" * 60)
    print("CATBOOST ENDER60 FAST COMPLETE")
    print("=" * 60)
    ...
    print(f"estimand: {result.manifest.get('estimand')}")
    if result.scorecard is not None:
        ...
        print(f"scorecard_mean_payout:    {sc.mean_payout.value:.5f}")
