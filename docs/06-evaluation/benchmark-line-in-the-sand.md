...
- **G — Tier-4 production gate:** measured on `v53_lgbm_ender60` over the shared meta-model overlap under `classic_atomic_ender60_r1343_v1` — CORR ≥ 0.0286, 60D AC-Sharpe ≥ 0.86, FNC@medium ≥ 0.020, and GPR ≥ 1.50. The measured reference is CORR 0.029270, 60D AC-Sharpe 0.867954, and GPR 47.7628. DSR lacks search-history identity, turnover is unavailable because consecutive validation eras share no IDs, and weekly-era CAGR is not an Atomic round-level capital measure; all three remain diagnostics and are absent from the hard-gate schema.
