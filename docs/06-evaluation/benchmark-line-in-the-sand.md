...
- **G — Tier-4 production gate:** measured on `v53_lgbm_ender60` over the shared meta-model overlap under `classic_atomic_ender60_r1343_v1` — CORR ≥ 0.0286, 60D AC-Sharpe ≥ 0.86, FNC@medium ≥ 0.020, and GPR ≥ 1.50. The 2026-09-03 refresh measured CORR 0.029270, 60D AC-Sharpe 0.535880, FNC 0.027278, and GPR 47.7628; the reference therefore fails the configured Sharpe hurdle, recorded in `artifacts/reports/benchmark_gate_report.csv`. DSR lacks search-history identity, turnover is unavailable because consecutive validation eras share zero IDs, and weekly-era CAGR is unavailable because it is not an Atomic round-level capital measure; these fields are absent from the hard-gate schema.
