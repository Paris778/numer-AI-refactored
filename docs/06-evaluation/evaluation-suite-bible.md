...
For the dashboard and scorecard capital context, `mean_payout` is this clipped series' arithmetic mean, reported with `mean_payout_ci_low`, `mean_payout_ci_high`, and `mean_payout_n_eras`. It is a policy-bound payout-per-scored-era proxy and may be formatted as a percentage; it is not an annual return, account CAGR, or settlement-performance claim. For Atomic comparisons, use the exact eligible `validation.parquet ∩ meta_model.parquet` era overlap, whose size is snapshot-dependent. Never multiply it by 52 or compound it to manufacture an annual rate.
...
**Mean payout per era (policy proxy)** (`mean_payout`). This is $\bar\pi = n^{-1}\sum_t r_t$ over the exact usable evaluation overlap, with bootstrap confidence bounds and `mean_payout_n_eras`. It is the honest percentage-like display available from weekly validation data: it describes the average policy payout score per evaluated era, not annualized capital growth. Atomic account CAGR remains unavailable until round-level scores, stake/allocation state, settlement timing, and claims are available. The dashboard labels this field `PAYOUT / ERA (PROXY)` and omits `cagr_1y` from the visible metric and dossier surfaces; the underlying scorecard field remains available only for valid legacy periodic diagnostics.

**CAGR 1Y** (`nmr.payout.annual_compounded_return`; scorecard column `cagr_1y`). For genuine periodic returns, geometric compounded growth is annualized at 52 observations/year: $\text{CAGR} = (\prod_t (1 + r_t))^{52/n} - 1$ when $\prod_t (1 + r_t) > 0$. A non-positive wealth product — total ruin, which the geometric form could not otherwise express — maps to $-1.0$. Fewer than 2 observations return 0.0. All arithmetic is float64 on the finite-only validated input (`_as_finite_1d`: 1-D, non-empty, finite-only, else `ValueError`). This helper is used for legacy periodic payout observations; Atomic weekly validation scorecards set `cagr_1y = None` because their era proxies cannot represent the five rounds per week, 64 concurrent positions, or account-level timestamps. The unavailable value carries `capital_metrics_reason="round_level_returns_unavailable"`.
