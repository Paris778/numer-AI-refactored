...
3. **Generate candidates** — subset definitions must be pure functions of `features.json` + screen output: same inputs ⇒ same subset. Screen-derived sets (`screen_stable`, `screen_nonlinear`, `screen_linear_or_nonlinear`, `screen_drift_filtered`) are materialized by the `derived_sets` analysis stage into `derived_feature_sets.json` and consumed via `data.supplemental_feature_sets` (merged by `IngestionAgent`; key collision raises). A different subset is a different experiment by design: the run_id fingerprint covers config (incl. `data.feature_subset` + the supplemental path) + data_version + `nmr/*.py` + environment (ARCHITECTURE.md §P).
