...
Every long job writes durable progress: stage markers, completed/total counts, and elapsed time per unit — enough to answer "how far along?" and "how much longer?" from the log alone (markers: `ARCHITECTURE.md` §V).
...
| Task | Look in... |
|---|---|
| config schema / valid values | `nmr/config.py` — frozen dataclasses, `load_config`, `VALID_*`; backend syntax here, registry resolution in `ModelOrchestrator` |
| experiment-layout path derivation | `nmr/paths.py` — pure layout: `validate_slug`, run/export/pointer paths, shared cache/report dirs (spec: `ARCHITECTURE.md` §X) |
...
| backend protocol / registry | `nmr/model_backend_protocol.py` + `nmr/model_backend_registry.py` — public backend types, canonical identity, sealed built-in registry |
| backend adapters / orchestration | `nmr/model_backend_*.py` + `nmr/models.py` — adapter-specific fit/predict/identity and `ModelOrchestrator` policy |
| selected checkpoint identity / HPO provenance | `nmr/_oof.py`, `nmr/research.py`, `nmr/opt.py` — selected fitting hash, checkpoint manifests, HPO provenance |
...
| HPO sweeps / neutralization frontier | `nmr/research.py` + `nmr/opt.py` — sweeps, held-out provenance, neutralization frontier |
...
| Model Tournament dashboard engine + shared renderer | `nmr/dashboard.py` + `dashboard_ui/`; `generate_dashboard.py`/`dashboard_app.py` are thin hosts (spec: `ARCHITECTURE.md` §W) |
...
| Promote a run to a full/partial export (`train_only` → partial + cross-check `scorecard.json`; Model Uploads `predict.pkl`) | `nmr/promote.py` + CLIs — promotion/rehearsal, acceptance gate, capital-evidence replay, pointer-repair receipt, custom-backend registry check |
...
`ModelOrchestrator` is GPU-first with CPU fallback for CV (`ARCHITECTURE.md` §G); manifest records `pipeline_device`/`oof_device`. `model.device` (`auto|gpu|cpu`) controls CV: `gpu` forces CUDA and `cpu` never attempts it. `run.workflow="gpu_screen"` is OOF-only; `cpu_confirm` is authoritative CPU validation. Research full-history validation uses `model.validation_fit_device`; deploy/promotion uses CPU via `model.deploy_fit_device`. Determinism is per-device, not cross-device.
...
- **cupy (`nmr/_gpu.rankdata`; measurements + NaN policy: `ARCHITECTURE.md` §U):** never import cupy into `nmr/_transforms` (embedded by value in deploy artifacts — must stay numpy/scipy-only).
...
- **Analysis runtime:** ~4–5 h full-universe (`--features all`); detail: the pre-modelling study §8.

### Full-version training is RAM-bound on this box (measured 2026-08-18)
- **Full version at medium is infeasible here** (numbers, curve, open slope hypothesis: `ARCHITECTURE.md` §O, `measure_ram_curve.py`). `promote_full_version`'s dual-metric RAM guard refuses (commit vs commit limit, WS vs physical); **Stage 2 is deferred** — the D7 Stage-1 truncated artifact is the accepted deliverable.
- **Full-universe training** (3,555): **only when the machine is otherwise idle** (the `lgbm_v1` campaign OOM: `_ArrayMemoryError: 28.1 GiB, shape (3555, 2123070)`).
- **Memory guards live in code** (`coerce_float32_features`, zero-copy views, era-batched predicts, spawned full-history worker + dual-metric RAM guard — full spec: `ARCHITECTURE.md` §G). OOF neutralization at 3,555 features is compute-bound (per-era pinv, ~3.5 h) — not memory, do not "optimize".
...
### OOF fold checkpoints (2026-08-20); full-history + validation checkpoints (2026-08-23)
Four checkpoint trees under `experiments/<slug>/runs/<run_id>/`: `oof_checkpoints/`, `validation_fit_checkpoints/`, `deploy_checkpoints/`, `validation_checkpoints/` — per-unit manifests bind code/device/target/feature/splitter identity (full spec: `ARCHITECTURE.md` §G). Any code/device/identity mismatch raises — delete the directory to force a refit (never silently reuse stale OOF). The final `evaluate_model` scorecard call stays uncheckpointed. Checkpoints are deleted with their run dir; clearing `experiments/*/runs/` remains ask-first.

### Ridge cell memory budget (D1a)
Medium Ridge cell peaked at 79.0 GiB commit / 49.7 GiB WS pre-fix (frozen refs `artifacts/reports/ridge_reference_*`). The memory-safe path keeps Ridge **bit-identical** — bounded block stats, column-block float32 matrices from polars (float32 inputs REQUIRED: sklearn cholesky is float32-on-float32; predict-dot promotion ≠ float64 matrix), gather-indexed sort, whole-block predict (chunked predict NOT bit-identical: BLAS call shape; rank blend amplifies ulp drift) — and preflights `ridge_memory_budget`: refuses on the 52 GiB ceiling (`NMR_RIDGE_COMMIT_CEILING_BYTES`), machine commit limit, or 0.85×physical WS guard. Post-fix: 25.1 GiB commit / 16.1 GiB WS (spec: §M). Never weaken these guards.

### Tier-0 null-floor calibration (2026-09-12)
The ±0.15 single-seed AC-Sharpe floor is retired: the gate checks the family-wise max vs the pre-registered p99 in digest-bound `configs/benchmarks/null_floor_calibration.json` (1000 seeds; window/identity bound; seed-42 anchor). Never hand-edit; regenerate via `null_floor_study.py` (spec: §M).

### Thread-pool caps must run at process start (2026-08-23)
Heavy CLIs, including `benchmark_runner.py` and `sklearn_breadth_runner.py`, call `nmr.hardware.apply_thread_limits()` first: it sets `POLARS_MAX_THREADS` / `OMP_NUM_THREADS` / `OPENBLAS_NUM_THREADS` / `MKL_NUM_THREADS` (env `NMR_MAX_THREADS`, default min(8, cores); user values win; invalid env raises). Never import before that call.
</operational_hazards>
