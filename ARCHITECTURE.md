...
- **Checkpoints & resume:** three stages checkpoint under `experiments/<slug>/runs/<run_id>/` — `oof_checkpoints/` (CV fold parts), `deploy_checkpoints/` (per-target pickled full-history models), `validation_checkpoints/` (per-era-batch prediction frames). Each unit's `manifest.json` records `code_sha256` (SHA-256 of `nmr/models.py` + `nmr/splitter.py` + `nmr/runner.py` source bytes — run_id binds config and data, never code), `device` (the post-fit `resolved_device`), the rebuild-identity terms `data_fingerprint`/`environment`, and the fit-identity terms `target_col` / canonical feature-list fingerprint / splitter-geometry fingerprint. A checkpoint copied from another target, feature list, or fold geometry therefore refuses resume. The manifest is written atomically at the **first completed unit** because the device is known only after fitting. OOF and deploy manifests are per-target; the validation manifest sits at the root. A code/device/identity mismatch, torn tree, or corrupt part raises `ValueError` with deletion guidance — stale checkpoints are never silently reused. Shared helpers live in `nmr/_oof.py` (`fitting_code_sha256`, `feature_list_fingerprint`, `splitter_geometry_fingerprint`, `checkpoint_manifest`, `verify_checkpoint_manifest`, `ensure_no_torn_tree`, `write_frame_atomic`, `write_bytes_atomic`); all writes go through `nmr/_atomicio.py::atomic_write_bytes`. Retention follows the run directory; concurrent duplicate run_ids are unsupported.
     ...
     - **Deploy fits:** `_build_deploy_pipeline(..., deploy_checkpoint_dir=None)` persists each fitted full-history model to `deploy_checkpoints/<target>.pkl` with a sibling identity manifest. Manifest-first ordering means a crash can leave only a manifest, which is safe to refit; an unidentified pkl is never left behind. Resume rebuilds the predict closure from loaded CPU models.
...
- The pre-registered forced-GPU XGBoost campaign uses `configs/xgb-gpu-calibration-ender60.yaml` as a short CUDA smoke/capacity run, followed by `configs/xgb-gpu-ender60-standard.yaml` (single Ender-60 specialist) and `configs/xgb-gpu-60d-ensemble-standard.yaml` (four 60D specialists blended in rank space). All use `small` features because the available RTX A1000 Laptop GPU has 4 GiB VRAM; the `all` universe and full-history deployment are outside this overnight protocol. The calibration must prove `oof_device="gpu"` before the standard candidates run. `validation_scorecard=true` also trains the CPU-only full-history prediction pipeline required to score validation, even when `deploy=False`; `deploy` additionally serializes the artifact. The runner snapshots the OOF device before that CPU fit so `manifest.oof_device` remains truthful. Candidate selection uses the existing validation scorecard and atomic 60D payout contract, with `mean_payout` as the primary comparison; same-window selection remains research evidence, not an unbiased live estimate.
