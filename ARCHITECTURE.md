...
| `model: ModelConfig` | `backend="lightgbm"`, `preset="fast"`, `params={}`, `device="auto"`, `validation_fit_device="cpu"`, `deploy_fit_device="cpu"` | backend is a syntax-stage identifier matching `^[a-z][a-z0-9_]*$` (the built-in help tuple is `("lightgbm", "xgboost", "catboost", "ridge")`; unknown syntactically valid names are rejected later when `ModelOrchestrator` resolves the selected registry adapter); preset ∈ `("fast", "standard", "deep")`; `device` ∈ `("auto", "gpu", "cpu")`; full-history validation fit ∈ `("gpu", "cpu")`; deployment fit must be `"cpu"` |
...
| `run: RunConfig` | `name="default"`, `seed=42`, `workflow="standard"`, `artifacts_dir=REPO_ROOT/"artifacts"` | workflow ∈ `("standard", "gpu_screen", "cpu_confirm")` |
...
**Research workflow contract:** `run.workflow="gpu_screen"` is a fast OOF-only research phase: it requires forced GPU CV, disables the validation scorecard, rejects `mmc`, and `ExperimentRunner.run(deploy=True)` fails before loading data. `run.workflow="cpu_confirm"` is the shortlist phase: it requires the validation scorecard and `validation_fit_device="cpu"`. Screen and confirmation configs should keep data, split, model parameters, ensemble, and risk settings identical; only the workflow/device/scorecard policy changes. This prevents spending CPU time until a GPU-screened idea is explicitly shortlisted.
...
`PurgedEraSplitter(split: SplitConfig).split(eras) -> list[Fold]`; `Fold(index, train_eras, val_eras)` frozen. `PurgedEraSplitter.purge_eras -> int` exposes `split.purge_eras` so consumers can re-assert the purge width at train time. `scoring_purge_eras(purge_eras, scoring_horizon) -> int` is the single source for the validation-window purge shared by the runner's validation stage and promotion's cross-check: `max(config purge, 8 for 20D / 16 for 60D policy overlap)`; `scoring_horizon` is the RAW policy horizon — legacy policies that bind none contribute no overlap.
...
All transforms follow the canonical definitions in [docs/01-canon/10-scoring-reference.md](docs/01-canon/10-scoring-reference.md).
...
Metric definitions follow the canonical tournament spec in [docs/01-canon/10-scoring-reference.md](docs/01-canon/10-scoring-reference.md). The repo's judging rules are the evaluation spec of record: [docs/06-evaluation/evaluation-suite-bible.md](docs/06-evaluation/evaluation-suite-bible.md).

### F. Neutralization — `nmr/risk.py`

`NeutralizationEngine(cache_dir=REPO_ROOT/"artifacts"/"cache"/"neutralization")` — the repo-global default; production callers with a config pass `cache_dir=paths.shared_cache_dir(config.run.artifacts_dir)/"neutralization"` (runner, research held-out path), so a custom `run.artifacts_dir` retargets the cache with the rest of the shared dirs.
...
Model-development support is intentionally split into three levels:

| Level | Path | Boundary |
|---|---|---|
| Level 1 | Foreign model scoring via `nmr/predictions.py` | Any process that emits canonical `(era, id, prediction)` rows can be research-scored without entering the backend registry or `nmr/models.py`. |
| Level 2 | First-class backend via `BackendAdapter` + explicit `BackendRegistry` | The model participates in purged OOF, checkpoint identity, and HPO plumbing, but remains research-only unless it also satisfies the Level 3 contracts. |
| Level 3 | Promotion/deployment-eligible backend | The backend satisfies unchanged capital-evidence, deployment, acceptance, and promotion rules; custom runs additionally require the matching explicit `BackendRegistry` during HPO/promotion flows. |

The backend layer is split by responsibility; adapters do not own folds, checkpoints, or validation windows:

| File | Owns |
|---|---|
| `nmr/model_backend_protocol.py` | `BackendCapabilities`, `BackendIdentity`, `BackendAdapter`, and canonical JSON identity normalization |
| `nmr/model_backend_registry.py` | `BackendRegistry`, built-in adapter mapping, name normalization, registration validation, snapshot sealing, and registry audit identity |
| `nmr/model_backend_lightgbm.py` | LightGBM-only param translation, build/fit/predict, and backend identity |
| `nmr/model_backend_xgboost.py` | XGBoost-only param translation, build/fit/predict, and backend identity |
| `nmr/model_backend_catboost.py` | CatBoost-only param translation, build/fit/predict, and backend identity |
| `nmr/model_backend_ridge.py` | CPU-only Ridge adapter, train-fold `StandardScaler` + `Ridge` state, zero-variance/finiteness guards, and serializable model state |
| `nmr/models.py` | `ModelOrchestrator`: device policy, fold/full-history loops, chunked prediction, checkpoint calls, and shared deploy inputs |

Exact backend protocol:
- `BackendCapabilities(supports_gpu, supports_full_history, supports_deployment, deployment_device)` is descriptive only; `deployment_device` is currently `"cpu"` or `"none"`, and `supports_deployment=True` requires `deployment_device="cpu"`.
- `BackendIdentity(schema_version=1, name, adapter_version, implementation_fingerprint, resolved_params, device, capabilities, dependency_identity)` is canonical JSON data. `name` must match `^[a-z][a-z0-9_]*$`, `implementation_fingerprint` must be a 64-character lowercase SHA-256, `device` is `"cpu"` or `"gpu"`, and `dependency_identity` is a sorted package-version mapping.
- `normalize_identity_value()` / `canonical_json_bytes()` are the single source of truth for canonicalization: mappings use string keys and sorted order; NumPy scalars normalize to native Python scalars; non-finite floats, `Path` objects, callables, estimator instances, object addresses, and runtime paths are rejected.
- `BackendRegistry.with_builtins()` registers `lightgbm`, `xgboost`, `catboost`, and `ridge`. `register()` validates the adapter protocol, lowercased name, `identity(...).name`, and capability agreement; `snapshot()` deep-clones mutable adapter state and seals the per-run registry; `identity()` sorts adapter names and records the full snapped registry as audit provenance.
- `ModelOrchestrator`, `ExperimentRunner`, `HyperparameterSweep`, `bayesian_sweep`, `promote_full_version`, and `rehearse_promotion` all accept the same optional `backend_registry=` seam. Built-in runs default to `BackendRegistry.with_builtins()`; a custom backend run must supply the matching explicit registry anywhere the selected adapter must be resolved again.

`ModelOrchestrator(config: ModelConfig, *, seed=42, backend_registry=None)`:
- construction snapshots the supplied `BackendRegistry` (or `BackendRegistry.with_builtins()`) and resolves the selected adapter exactly once, so unknown syntactically valid backend names fail at construction before data loading or fit; built-in tree adapters live in `nmr/model_backend_lightgbm.py`, `nmr/model_backend_xgboost.py`, and `nmr/model_backend_catboost.py`, and the CPU linear protocol-test backend lives in `nmr/model_backend_ridge.py`
...
- `train_full_history(df, *, feature_cols, target_col, era_col="era", include_validation=False, fit_device="cpu") -> model` — one explicit-device model fit on every era, with null/non-finite targets dropped (logged count; `ValueError` if nothing remains). `fit_device="gpu"` is for research-only validation and is forced without CPU fallback; deployment/promotion callers retain `fit_device="cpu"` so artifacts reproduce on CPU-only hosted runtimes. `include_validation` (promotion writer): when the fit spawns, the child re-reads train+validation concatenated so the full version sees the validation eras.
...
- Backend ownership split: concrete adapters own backend-specific param translation, estimator construction, fit invocation, predict invocation, capabilities, and identity; `ModelOrchestrator` still owns device-candidate ordering, GPU-first/CPU-fallback policy, fold/full-history loops, chunked prediction, checkpoints, and the shared deploy closure inputs.
- OOF-CV is GPU-first for lightgbm/xgboost with automatic CPU fallback: LightGBM via `device_type="gpu"`, XGBoost (>= 3.0) via `device="cuda"` + `tree_method="hist"` (`gpu_hist` was removed in 3.x and raises `Invalid Input`). A failed device attempt is logged with `type(exc).__name__` + message (only backend errors — `ValueError`/`TypeError`/`LightGBMError`/`XGBoostError`/`CatBoostError` — trigger fallback; anything else fails loudly), and `resolved_device` records which device actually fit (`"gpu"`/`"cpu"`, `None` before the first successful fit). `model.device` (`auto` | `gpu` | `cpu`, default `auto`) controls CV/experimentation: `gpu` returns only the GPU candidate (a failure raises — no silent fallback), `cpu` never attempts GPU. `model.validation_fit_device` explicitly controls the research full-history fit used for validation; `model.deploy_fit_device` is constrained to `cpu` for deployment/promotion. The run manifest records the config device as `pipeline_device`, the actual CV fit as `oof_device`, the full-history roles separately, the selected backend identity that enters `run_id`, and the full registry audit identity as provenance only. CatBoost is CPU-only by design — it never attempts a GPU candidate (see below).
- **Checkpoints & resume:** four stages checkpoint under `experiments/<slug>/runs/<run_id>/` — `oof_checkpoints/` (CV fold parts), `validation_fit_checkpoints/` (research full-history per-target models), `deploy_checkpoints/` (CPU deployment per-target models), and `validation_checkpoints/` (per-era-batch prediction frames). Each unit's `manifest.json` records `code_sha256` (the selected fitting hash from `nmr._oof.fitting_code_sha256()`: SHA-256 of the shared fitting path `nmr/model_backend_protocol.py` + `nmr/model_backend_registry.py` + `nmr/models.py` + `nmr/splitter.py` + `nmr/runner.py`, plus only the selected adapter source file(s)), `device` (the post-fit `resolved_device`), the selected backend identity (registry schema version, selected backend name, adapter version, selected source fingerprint, resolved-params identity, device role, capabilities, dependency identity), the full registry audit identity for provenance only, the rebuild-identity terms `data_fingerprint`/`environment`, and the fit-identity terms `target_col` / canonical feature-list fingerprint / splitter-geometry fingerprint. A checkpoint copied from another target, feature list, fold geometry, or selected backend identity therefore refuses resume; unrelated adapter additions do not. The manifest is written atomically at the **first completed unit** because the device is known only after fitting. OOF, validation-fit, and deploy manifests are per-target; the validation manifest sits at the root. A code/device/identity mismatch, torn tree, or corrupt part raises `ValueError` with deletion guidance — stale checkpoints are never silently reused. Shared helpers live in `nmr/_oof.py` (`fitting_code_sha256`, `feature_list_fingerprint`, `splitter_geometry_fingerprint`, `checkpoint_manifest`, `verify_checkpoint_manifest`, `ensure_no_torn_tree`, `write_frame_atomic`, `write_bytes_atomic`); all writes go through `nmr/_atomicio.py::atomic_write_bytes`. Retention follows the run directory; concurrent duplicate run_ids are unsupported.

Custom backend promotion boundary:
- Promotion and rehearsal do not serialize executable registries, discover adapters from import paths, or trust process-global registration. Built-in runs resolve through the built-in registry; a custom run must pass the matching explicit `BackendRegistry` into HPO, promotion, rehearsal, and any pointer-repair flow that needs to re-resolve the selected adapter.
- Promotion eligibility is layered: Level 2 custom adapters may run inside OOF/checkpoint/HPO flows, but Level 3 promotion additionally requires `supports_full_history=True`, `supports_deployment=True`, authoritative capital evidence, and the existing deploy acceptance checks. `SweepResult` or a research `evaluate_model()` call remains non-capital evidence.
     - **OOF folds:** `train_oof_with_checkpoints(df, *, feature_cols, target_col, splitter, era_col="era", checkpoint_dir) -> pl.DataFrame` loops over `oof_checkpoints/<target>/fold_NN.parquet` with a per-target manifest. `train_cross_validation` delegates to the same private `_cv_fold_parts` loop without checkpoints. `nmr/_oof.py::train_multi_target_oof(..., checkpoint_dir=None)` routes each target here when configured. Mixed load-and-refit resume is bit-for-bit identical to a fresh fit; fold disjointness remains enforced.
     - **Full-history fits:** `_build_deploy_pipeline(..., deploy_checkpoint_dir=None, checkpoint_kind=..., fit_device=...)` persists each fitted full-history model to the selected per-target checkpoint tree with a sibling identity manifest. Research validation uses `validation_fit_checkpoints/` and may use the explicit GPU device; deployment uses `deploy_checkpoints/` and is CPU-only. Manifest-first ordering means a crash can leave only a manifest, which is safe to refit; an unidentified pkl is never left behind. Resume rebuilds the predict closure from models fitted on the requested device.
     - **Validation predicts:** `_run_validation_stage(..., validation_checkpoint_dir=None, checkpoint_device=None)` uses `_predict_validation_era_batches`, persisting `validation_checkpoints/preds_batch_NN.parquet` with a root manifest. Resume loads present batches and predicts missing ones byte-identically. A missing checkpoint device raises. The final `evaluate_model` call is not checkpointed because it has no useful intermediate boundary.
...
Presets mirror Numerai's published benchmark params and walk-forward purge convention in [docs/01-canon/03-models.md](docs/01-canon/03-models.md).

**Dynamic colsample floor:** the feature-sampling fraction is raise-only floored per feature count at fit time — `c_effective = min(1.0, max(c_resolved, min(1.0, max(0.1, min(10, |S|)/|S| + 1e-7))))`. The `1e-7` expansion guards float32 truncation in C++ backends and sits inside the `max(0.1, …)` bound, so `|S| ≥ 100` configs retain their resolved value. Applied in `_resolved_params` on the backend-final sampling keys: `colsample_bytree` for XGBoost; every present LightGBM alias in `{colsample_bytree, feature_fraction, sub_feature}`; and `rsm` after CatBoost translation. `n_features` is threaded `_fit_model → _device_candidate_params → _resolved_params`; both device candidates receive the same floor. The Optuna baseline anchor enqueues the raw resolved value (§S).
...
### Prediction artifacts — `nmr/predictions.py`

Validated prediction frames plus provenance so evaluation can consume a parquet that did not come from `nmr.models`. Closed stages: `PREDICTION_STAGES = ("raw", "blended", "neutralized", "validation", "submission")`. Closed partitions/roles: `ERA_PARTITIONS = ("oof", "validation", "live", "held_out")`, `FIT_ROLES = ("cv_oof", "full_history", "foreign")`. `PredictionProvenance` is frozen (stage, trained/ensemble/scoring targets and horizons, payout policy id, era partition, data/feature fingerprints, fit role, device, source run id, `selection_bias`, `split_estimand`, `validation_window`) — no wall-clock or paths; unknown closed-set values raise. `prediction_set_from_frame` / `read_prediction_set` require unique `(era, id)`, finite predictions, non-null keys, and sort `["era", "id"]`; submission stage additionally requires every value in `(0, 1)`; the stored prediction column is `Float64`. `compose_predictions` is the reusable composition entry: `Ensembler.blend` then optional `NeutralizationEngine.neutralize` (proportion must be finite in `[0, 1]`; missing feature columns may be inner-joined from an explicit `features` frame, and the joined keys must equal the prediction keys exactly), then optional per-era `tie_kept_rank` to `(0, 1)` for stage `submission`. `as_submission` is a rank transform, not a capital promotion — OOF/held-out partitions are rejected. `validation_key_fingerprint` is the canonical SHA-256 over the sorted `(era, id)` key universe.

**Capital authority model.** `evaluate_prediction_set` capital evaluation requires a `CapitalContext` derived by a trusted validation loader — the runner computes every field from the loaded, purged `validation.parquet` (window, key fingerprint, row count) and the run config (payout policy id, scoring target/horizon, data/feature fingerprints, training estimand). The predicate then requires `stage="validation"`, `era_partition="validation"`, `selection_bias=False`, all identity fields present, provenance matching the context on payout/target/horizon/training estimand, and **exact `(era, id)` coverage** across predictions, meta model, features, targets, and the optional benchmark frame when supplied — sparse, extra, or duplicate rows in any auxiliary frame refuse. The final joined base is checked after the benchmark join against the same row count and key fingerprint. Research callers retain inner-join behavior; only the capital wrapper activates the strict universe contract. `submission` is not capital-eligible; research stages return `ResearchEvaluation(is_capital=False)` only with `allow_research_stage=True`. Does not import `nmr.models`.

**Evidence chain.** The runner persists `manifest.capital_evidence` (`CAPITAL_EVIDENCE_VERSION`, version 1): window, key fingerprint, row count, payout/target/horizon, data/feature fingerprints, training estimand, and the digest of the stored scorecard (`nmr.scorecard.scorecard_block_digest`, timing stripped). Promotion (`nmr.promote`) independently derives `all validation eras - scoring_purge_eras(config.purge_eras, payout_policy.scoring_horizon)`, requires the evidence window to equal that result, binds `scorecard.n_eras` to its length, and then recomputes the exact `(era, id)` universe. Legacy runs without evidence are not promotion-eligible. Fresh publication and force-repair use the same authorization; publication also writes an immutable `promotion_authorization.json` receipt under the run directory, and repair binds export activation flags, evidence, scorecard, gate, and acceptance fields to that receipt before repointing. Direct `evaluate_model` callers (benchmarks, fleet, HPO, cross-check replay) produce research/benchmark scorecards — never capital evidence. `ExperimentRunner._run_validation_stage` scores through `evaluate_prediction_set`; OOF composition still calls `Ensembler` / `NeutralizationEngine` directly so OOF bytes stay bit-identical. **Threat-model boundary:** the evidence chain is anti-drift, not anti-forgery — it detects stale, partial, or mislabeled evidence and hand-edited inconsistencies; it does not authenticate a producer with filesystem write access to `experiments/` and read access to the data (every fingerprint is computable from public code + disk).
...
`mean_payout` and its confidence bounds are the arithmetic mean of the policy-clipped per-era payout series, with `mean_payout_n_eras` recording the actual usable era count. For Atomic evaluations this is a policy payout proxy per scored era, suitable for percentage formatting but not an annual return, account CAGR, or settlement-performance claim.

Weekly validation eras cannot reconstruct Atomic's five-round-per-week, 64-position portfolio. Atomic account-level capital metrics, including `cagr_1y`, are therefore `None` with `capital_metrics_reason="round_level_returns_unavailable"`; the legacy simulator remains an explicitly historical diagnostic, not Atomic capital evidence. The generic annual-compounding helper remains valid for genuine periodic return observations.
...
- `HyperparameterSweep(base_config, *, metric="sharpe").run(space, *, n_trials, seed) -> SweepResult` — Cartesian product of the space, shuffled, sampled to `n_trials`; each trial overrides `model.params` and evaluates on a purged held-out split (final ~20% of eras). `SweepResult` is a labelled research proxy (`is_capital=False`, `proxy_split="held_out_80_20"`, `proxy_metric`, `proxy_target`, `selection_bias=False`) — never a capital scorecard.
...
- The pre-registered forced-GPU XGBoost campaign uses `configs/xgb-gpu-calibration-ender60.yaml` as a short CUDA smoke/capacity run, followed by `configs/xgb-gpu-ender60-standard.yaml` (single Ender-60 specialist) and `configs/xgb-gpu-60d-ensemble-standard.yaml` (four 60D specialists blended in rank space). All use `small` features because the available RTX A1000 Laptop GPU has 4 GiB VRAM; the `all` universe and full-history deployment are outside this overnight protocol. The calibration must prove `oof_device="gpu"` before the standard candidates run. `validation_scorecard=true` builds the full-history prediction pipeline using `model.validation_fit_device` when `deploy=False`, and uses the CPU `model.deploy_fit_device` pipeline when `deploy=True`; `deploy` additionally serializes the artifact. The runner snapshots the OOF device before either full-history fit so `manifest.oof_device` remains truthful. Candidate selection uses the existing validation scorecard and atomic 60D payout contract, with `mean_payout` as the primary comparison; same-window selection remains research evidence, not an unbiased live estimate.
- Fast screening protocol: `xgb-gpu-screen-ender60.yaml` and `xgb-gpu-screen-60d-ensemble.yaml` run GPU OOF only (`validation_scorecard=false`) and record research metrics without paying for a full-history fit. Their matching `xgb-cpu-confirm-*.yaml` configs keep model/data/split parameters identical, switch `validation_fit_device` to `cpu`, and enable the authoritative validation scorecard. Only shortlisted GPU screens should enter CPU confirmation; the screen ranking is a search result and must not be reported as final deployment evidence.
...
three hard gates: `assert_tier0_null_floor` (structural |CORR| ≤ 0.005 and
family-wise max |AC-Sharpe| ≤ the **pre-registered p99** of the committed,
digest-bound null-floor calibration; seed-42 observed values anchored to the
stored expectations; no DSR check — null DSRs span 0.11–1.0),
`assert_tier4_gate` (four Atomic Ender-60 thresholds in `configs/benchmarks/tier4_gate.yaml`: CORR, 60D AC-Sharpe, FNC@medium, and GPR; DSR and turnover remain diagnostics because their required search or shared-ID evidence is unavailable, while weekly-era CAGR is unavailable rather than an Atomic capital measure),
...
**Ridge cell memory safety (D1a, 2026-09-08).** The medium Ridge cell previously peaked at **79.0 GiB commit / 49.7 GiB working set** (frozen pre-fix reference, `freeze_ridge_reference.py`): a full-width float64 deviation matrix (~15.7 GiB) inside `np.std(dtype=float64)`, full-width fancy-index subset copies, stacked polars cast frames, and a sorted-frame copy. The memory-safe path bounds every transient while keeping sklearn `Ridge` **bit-identical** to the pre-fix output (frozen-reference test: `tests/test_benchmark_ridge.py::test_frozen_ridge_reference_matches_memory_safe_path`, digest-equality asserted):
- `generate_ridge_predictions` preflights with `ridge_memory_budget` (dual-metric: configured commit ceiling `_RIDGE_COMMIT_CEILING_BYTES` 52 GiB, env override `NMR_RIDGE_COMMIT_CEILING_BYTES`; machine commit limit; working-set guard `_RIDGE_WS_FRACTION` 0.85 of physical) and **refuses before materializing any feature matrix** — an over-budget cell fails clearly instead of OOM-ing mid-receipt.
- Statistics: bounded two-pass column blocks (`_RIDGE_STANDARDIZE_BLOCK_COLS` 32) straight from the polars frame — no full-width numpy block for statistics; per-block float64 mean/std are bit-identical to the pre-fix whole-block computation.
- Feature matrices: the fit matrix (C-order float32) and the validation matrix (F-order float32) are each assembled column-block by column-block from the polars frame (int8 `to_numpy` → `astype(float32)`, exact for the v5.x integer bins) — no full-width raw numpy block, no fancy-index copy, no polars cast frame, no sorted-frame copy (the pre-fix `sort([era, id])` order is reproduced with a gather index; the rank blend's `np.std` pairwise reduction is order-dependent at the last ulp).
- **float32 inputs are REQUIRED for bit-identity, both proven by frozen-reference drift:** sklearn's Ridge cholesky runs in float32 arithmetic on float32 input (no float64 precomputation reproduces it), and numpy's float32→float64 promotion inside the predict dot is not bit-equal to a pre-built float64 matrix.
- Prediction stays a **whole-block call** (exactly the pre-fix call shape): per-row BLAS results depend on the call shape, and the rank-domain blend amplifies any last-ulp drift to O(1) in the final output — era-chunked predict is therefore NOT bit-identical and is forbidden on this path.

Measured post-fix (frozen `_post` reference): **25.1 GiB commit / 16.1 GiB working set**, predictions digest-identical to pre-fix. The pure estimator `estimate_ridge_peak_bytes` counts the per-array terms conservatively (`_RIDGE_ESTIMATE_SAFETY_FACTOR` 1.1) — the estimate deliberately overcounts the measured peak, never undercounts it.

**Tier-0 null-floor calibration (2026-09-12).** The single-seed `|AC-Sharpe| ≤ 0.15` floor was retired after `null_floor_study.py` showed the 86-era window's family-wise max |AC-Sharpe| has p50 ≈ 0.13, p90 ≈ 0.26, p95 ≈ 0.30, p99 ≈ 0.37 over 200 seeds — a fixed floor rejects roughly half of all seeds (seed-42 measured 0.215 for `null_gaussian_rand`), which is a calibration defect, not a metric break. The gate is now a **pre-registered family-wise quantile** (`method="family_wise_quantile"`, 1,000 fixed seeds, p99) stored in `configs/benchmarks/null_floor_calibration.json`:
- **Digest-bound and immutable:** the artifact carries a SHA-256 over a canonical JSON rendering (excluding the `digest` key itself); `load_null_floor_calibration` re-derives and compares it and REFUSES on schema drift, tampering, non-structural kind sets, or malformed values. Never hand-edit — regenerate through the pre-registered procedure.
- **Window-bound:** `verify_null_floor_window` (called in `BenchmarkHierarchy.__init__`, i.e. before any scoring) compares the calibration's `window_key_fingerprint` (`validation_key_fingerprint` over `meta_model.select(["era", "id"])`), its era list, and its scoring identity (`payout_policy_id`, `scoring_target`, `scoring_horizon`, `scoring_backend`) against the live run. A data-window or policy change REFUSES the suite until recalibrated.
- **Config-driven:** the tier-0 YAML declares `null_floor: {calibration: null_floor_calibration.json, corr_tol: 0.005}`; the calibration path resolves relative to that config file. No threshold literal remains in code.
- **Two-sided check:** observed seed-42 values must match the calibration's stored `seed42_expected` (determinism anchor, atol 1e-9) AND the family-wise max |AC-Sharpe| must sit at or below the calibrated p99; `null_feature_mean` stays excluded (feature-derived, not structural noise).
- **Reporting:** `NullFloorSummary` (method, quantile, seed count, threshold, observed values, reference digest) is stored on `BenchmarkHierarchyResult.null_floor_summary` and emitted as `null_floor_*` rows/columns by `gate_report_frame`.
- **Recalibration procedure:** re-run `null_floor_study.py --sweep-seeds 1000 --emit-calibration configs/benchmarks/null_floor_calibration.json`, review the binding evidence and quantiles, then regenerate the receipt. A failed receipt cannot be waived.
...
| [train_catboost_ender60.py](train_catboost_ender60.py) | Capital-path CatBoost Ender-60 (medium, canonical-fast 2000/depth-5): `load_config("configs/catboost-ender60-fast.yaml")` → `ExperimentRunner.run(deploy=False)` → `experiment_store.record_run_result`; prints OOF + validation scorecard + `manifest.estimand` |
...
| [sklearn_breadth_runner.py](sklearn_breadth_runner.py) | Attempts the public sklearn regression catalog on NumerAI medium with era-purged data, per-method subprocess timeouts/checkpoints, and timing telemetry; writes `artifacts/reports/sklearn_breadth_screen.csv` by default |
...
| [measure_ram_curve.py](measure_ram_curve.py) | Three-point full-history **commit** curve (measured, not estimated): runs real worker fits at increasing row counts in fresh subprocesses, fits `peak = a + b·rows`, and writes `artifacts/reports/ram_curve.json` (intercept, slope, R², extrapolation factor, both anchors). All logic in `nmr/promote.py` / `nmr/models.py`. **Measured (2026-08-18):** machine = 63.7 GiB RAM / 148.8 GiB commit limit. `commit = 2.57 + 8.87e-6·rows` GiB (R² = 0.993), `ws = 1.10 + 8.18e-6·rows` GiB; full-version (6.85M rows) extrapolates to combined commit ≈ 61–65 GiB, working set ≈ 55–58 GiB (86–90% of physical). **Stage 2 (full-history promotion at medium) is deferred on these numbers** — the D7 Stage-1 truncated artifact is the accepted deliverable. **Open hypothesis (next cycle):** the per-row slope implies ~2.5 float32-equivalents held simultaneously — suspects sklearn `check_array` copying the polars→numpy view and LightGBM `Dataset` construction (`lgb.Dataset(..., free_raw_data=True)`); target slope ~5e-6 GiB/row → full-scale commit ≈ 40 GiB (own determinism proof required). |
| [freeze_ridge_reference.py](freeze_ridge_reference.py) | D1a ridge-reference freezer: captures the medium Ridge cell's statistics, prediction digest, per-era CORR, and measured peak memory into `artifacts/reports/ridge_reference_<sha>_{pre,post}.json` (`--label`); the frozen pre/post pair is compared by the tolerance/digest test in `tests/test_benchmark_ridge.py` (spec: §M) |
...
`derive_feature_sets(screen, drift, *, primary_target=None) -> dict[str, list[str]]` — pure function of the train-only IC screen and drift profile. Returns the four keys `screen_stable`, `screen_nonlinear`, `screen_linear_or_nonlinear`, `screen_drift_filtered` (sorted lists). Primary target defaults to `"target"` when present, else the first distinct screen target. Features without a drift row are kept. Empty sets are valid. `analyze_dataset.py` `_stage_derived_sets` is a thin reader/writer over this function.
...
`SweepResult` is the shared frozen dataclass (§L): `trials`, `best_params`, `best_value`, backend provenance (`backend`, `adapter_version`, canonical `backend_identity`, `resolved_params_identity`, `training_geometry`), plus proxy labels `is_capital=False`, `proxy_metric`, `proxy_split="held_out_80_20"`, `proxy_target`, `selection_bias=False`. The backend provenance comes from the planned selected adapter identity on the snapped registry before fitting; it is research provenance only. A proxy can select research candidates; only a capital `evaluate_model` scorecard authorizes promotion.

`HGBHPOConfig` / `bayesian_hgb_sweep` tune the promising sklearn
`HistGradientBoostingRegressor` directly on the medium universe. The bounded
eight-parameter space covers learning rate, iterations, leaves, depth, minimum
leaf size, L2 regularization, bins, and feature subsampling. Trials maximize
mean per-era CORR on the leading tuning eras; trailing eras are holdout-only,
and the selected parameters are then refit uncapped for full-window CORR and
FNC confirmation. Each trial records fit/predict/score/total seconds and peak
RSS in an atomic checkpoint; Optuna remains single-threaded (`n_jobs=1`).
`VotingHPOConfig` / `bayesian_voting_sweep` applies the same search to a
`Ridge` + HGB `VotingRegressor` and adds the Ridge vote weight as a ninth
parameter. CLI: `python sklearn_hpo_runner.py` or
`python sklearn_voting_hpo_runner.py` ->
`artifacts/reports/sklearn_hgb_hpo_trials.csv` plus the confirmation CSV.
...
`nmr/dashboard.py` — Model Tournament engine — unified leaderboard, explicit metric directions with default `mean_payout` ranking (policy-clipped Payout / Era proxy, descending), source/tier-derived cohorts, deterministic rank maps, ML ADVANTAGE comparisons, compact immutable model dossiers, tier-4 gate projection, stored-first capital recompute, 7-metric multimetric timeseries, and pairwise rank-gaussian similarity. `mean_payout` is the first metric in the public dashboard metric contract. The initial leaderboard order and top-three medal accents use this primary metric; alternate metric selections remain available. The main percentage column is the policy-clipped `mean_payout` per scored era, labeled as a proxy; it is not annualized. Atomic `cagr_1y` is unavailable without round-level returns and timestamps, so it is omitted from the dashboard metric list, rank payload, and dossier; the underlying schema and scorecard retain it only for valid legacy periodic diagnostics. Benchmark scorecard CSVs must carry `payout_policy_id`, `scoring_target`, and `scoring_horizon`; missing identity is rejected rather than silently compared. The row payload exposes that identity and the renderer shows a compact policy context beside each model so policy-scoped ranks remain legible. The tier-4 reference row is rejected if its identity does not match the configured gate. Custom registry roots are propagated to leaderboard and timeseries scans; they never fall back to the process-wide experiments root. `UNIFIED_SCHEMA` carries the mean-payout confidence/coverage fields and family lifecycle facts (`display_name`, `lifecycle_stage`, `current_full_status`, `stale` — spec §8). `dashboard_ui/charts.py` provides tested SVG geometry plus compact columnar payload encoding; `dashboard_ui/static/{layout.html,app.js,style.css}` is the single leaderboard-first renderer for search, rank switching, cohort filters, shortcuts, landscape/profile charts, and the model dossier drawer. `dashboard_ui/report.py` compiles the deterministic single-file HTML using generated `app.min.js` and `style.min.css`; `dashboard_ui/app.py` embeds the same HTML with `st.components.v1.html`. The report is explicitly offline evaluation and contains no live or production performance. Presentation tests live in `tests/test_dashboard_ui.py`.
The shared `dashboard_ui.service` layer uses the same `DEFAULT_RANK_METRIC` for `LeaderboardFrame.sort_by_metric`, `prepare_leaderboard_for_display`, and `compute_top_performers`; typed top-performer rows retain `mean_payout` alongside the supporting decision metrics.

Dashboard cohorts are presentation-derived, not a new registry field: trained rows are `source in {trained, trained_legacy}`; heuristic rows are the named null/statistical programmatic baselines (`null_constant_05`, `null_feature_mean`, `null_gaussian_rand`, and `null_uniform_rand`); Ridge, tree, canonical, and official reference experiments are benchmark rows; and `source in {full, partial}` rows are diagnostic-only with null comparable metrics — never ranked (full = in-sample, partial = train-only cross-check). Every rank metric declares whether higher or lower is better; nulls sort last and ties use `model_id`. The ML ADVANTAGE strip compares the best trained row with the best available heuristic and benchmark rows on the active metric. RAPS and win-rate are not active scorecard fields and are omitted rather than inferred.

Training recency is recorded in each completed run manifest as the timezone-aware UTC `trained_at` timestamp. The dashboard projects `trained_at` and derives `is_new` after scanning all rows: exactly the newest timestamped `trained`/`trained_legacy` row is marked `true`, with `model_id` as the deterministic tie-breaker; benchmarks and diagnostic exports are never marked. Missing or malformed legacy timestamps remain unmarked.

**Type & tier badges (presentation-derived, in the row payload):** `row_type_labels()` / `row_type_label()` and `row_tier_label()` (exported from `nmr`) stamp every payload row with the renderer's badge tokens. `type_labels` is the ordered badge STACK (the renderer shows one pill per token): benchmark-hierarchy rows LEAD with their cohort tag — `benchmark` (Ridge, tree, canonical, and official reference experiments) or `heuristic` (the named null/statistical baselines) — then resolve the model kind by stable `model_id` (`_BENCHMARK_KINDS_BY_ID`) to `null` / `ridge` / `lgbm` / `xgb` (+ an `ensemble` modifier for the multi-target Ridge / Sunshine blends), e.g. `[benchmark, lgbm]`, `[heuristic]`. Trained rows carry no cohort tag: family/backend resolves to a base kind (`lgbm`, `xgb`, `catboost`, `ridge`, or `trained`) with `ensemble` appended when the name contains "ensemble". `type_label` is the primary (first) token, used as the Type-column sort key and single-badge fallback — so Type-sorting groups benchmark rows, then heuristic rows, then trained kinds. `full`/`partial` keep their diagnostic identity. `tier_label` (`Tier 0`…`Tier 4`) badges every benchmark-hierarchy row (both heuristic and benchmark cohorts) and is `None` for trained rows. Benchmark display names are curated per stable `model_id` in `_BENCHMARK_HUMAN_NAMES` (e.g. `null_constant_05` → "Constant 0.5", `v53_lgbm_ender60` → "Ender 60D"); `run_name` keeps the full `Tier 4 · Ender 60D` form for series/bar labels, while `display_name` carries the short curated name the model cell badges against. The renderer renders the type as a stack of colored rounded pills (`.type-badge` / `.type-<kind>`, cohort tag first) and the tier as a colored tag (`.tier-badge` / `.tier-<n>`, gold at tier 4) inside the model cell; the model drawer and profile label use the rich `modelTitle()` (tier context preserved).

**Model dossier description:** every payload row carries a human-readable `description` ("what is this model?"), rendered as an "About this model" block at the top of the drawer card. `model_description()` returns a curated blurb per benchmark `model_id` (`_BENCHMARK_DESCRIPTIONS`, grounded in `configs/benchmarks/*.yaml`), a derived factual description for trained rows (backend, preset, feature set, target count, neutralization, ensemble flag), or explicit diagnostic wording for full/partial exports.

**Sortable leaderboard columns:** every header is clickable (`.sortable`). Clicking a column cycles best-first → reversed → back to the default rank ordering; the Rank header resets the sort; Model/Type sort textually, Eras and metric columns sort numerically, and nulls always sort last. Active sort shows `aria-sort` plus a `▲/▼` indicator, and full/partial diagnostic rows stay pinned after evaluable rows.
...
recorder (the retired `registry.record`): the run-immutability preflight runs
FIRST (a re-record whose payload differs from the recorded run.json raises
before any write), then it persists `oof.parquet` and
...
model_backend_protocol.py (leaf — backend protocol types + canonical identity normalization)
model_backend_lightgbm.py (leaf — LightGBM adapter: params/build/fit/predict/identity)
model_backend_xgboost.py (leaf — XGBoost adapter: params/build/fit/predict/identity)
model_backend_catboost.py (leaf — CatBoost adapter: params/build/fit/predict/identity)
model_backend_ridge.py (leaf — Ridge adapter: scaler + ridge state, params/build/fit/predict/identity)
...
predictions.py ──> _transforms (tie_kept_rank), ensemble, risk, scorecard
payout.py    ──> inference
...
model_backend_registry.py ──> model_backend_protocol (registry), model_backend_lightgbm/xgboost/catboost/ridge (lazy built-in loading)
models.py    ──> config (ModelConfig), data, model_backend_registry, splitter (Fold, PurgedEraSplitter)
