...
| `model: ModelConfig` | `backend="lightgbm"`, `preset="fast"`, `params={}`, `device="auto"`, `validation_fit_device="cpu"`, `deploy_fit_device="cpu"` | backend ∈ `("lightgbm", "xgboost", "catboost")`, preset ∈ `("fast", "standard", "deep")`; `device` ∈ `("auto", "gpu", "cpu")`; full-history validation fit ∈ `("gpu", "cpu")`; deployment fit must be `"cpu"` |
...
`SweepResult` is the shared frozen dataclass (§L): `trials`, `best_params`, `best_value`, plus proxy labels `is_capital=False`, `proxy_metric`, `proxy_split="held_out_80_20"`, `proxy_target`, `selection_bias=False`. A proxy can select research candidates; only a capital `evaluate_model` scorecard authorizes promotion.
