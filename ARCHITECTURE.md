...
- **Memory discipline (recorded 2026-08-10):** `coerce_float32_features(df, feature_cols)` casts exactly-representable schemas (Int*/UInt*/Float32 only — the v5.x integer bins) to a single Float32 block **before** pandas, so LightGBM/XGBoost's `to_numpy(dtype=float32)` is a zero-copy view; Float64/mixed schemas pass through untouched (precision is never silently dropped). All fit/predict feature frames go through it (`_feature_frame`). At 3,555 features × 2.1M rows the avoided copy is ~28 GiB (the `lgbm_v1` campaign OOM). The **validation stage** predicts in era-batches (`runner._predict_in_era_batches`, 40 eras/chunk) so the deploy closure's float64 `to_numpy` stays ≤ ~1.7 GiB instead of ~54 GiB. Both are bit-identical to the pre-optimization paths (Int8 bins exact in float32; per-era closure ops order-independent) — determinism tests cover them.
...
| [analyze_dataset.py](analyze_dataset.py) | Modular dataset analysis: 16 named stages (`overview`, `targets`, `ic_by_era`, `screens`, `summary`, `psi`, `drift`, `derived_sets`, `corr_medium`, `corr_all`, `set_membership`, `ic_by_split`, `regimes`, `benchmarks`, `meta_ortho`, `manifest`). Flags: `--only a,b` / `--skip a,b` run a subset (dependencies auto-included; `manifest` always runs), `--features small\|medium\|all`, `--max-eras N`, `--full-all-matrix`. `drift` writes the PSI + W1 + adversarial-AUC profile (`feature_drift_profile.parquet`, `w1_norm = w1 / σ_train`); `derived_sets` writes `derived_feature_sets.json` (`screen_stable`, `screen_nonlinear`, `screen_linear_or_nonlinear`, `screen_drift_filtered` — pure functions of the screen + drift dumps, sorted; see §P); `meta_ortho` writes per-feature meta-model orthogonality; the FNE profile uses an 11-point neutralization grid. Stage boundaries and per-era ticks print progress to stdout/stderr (never into artifacts); the manifest records `stages_run` + a machine-hardware summary (informational — never hashed). |
...
`feature_ic_screen(...) -> pl.DataFrame` — the chunked analysis twin of `feature_stability_screen` (§P), adding `mean_spearman` (mean per-era Spearman rank IC over valid eras), `nonlinear` (`|Pearson| ≤ min_mean_corr` AND `|Spearman| > min_mean_corr`), and the **UQ columns** `mean_corr_ci_lo` / `mean_corr_ci_hi` — 95% stationary block-bootstrap CI (20D block convention, `resolve_block_len`, seeded `IC_CI_SEED`) on the era-mean Pearson IC over valid eras. A feature whose CI spans zero cannot be called stable at 95% confidence.
