...
Validated prediction frames plus provenance so evaluation can consume a parquet that did not come from `nmr.models`. Closed stages: `PREDICTION_STAGES = ("raw", "blended", "neutralized", "validation", "submission")`. `PredictionProvenance` is frozen (stage, trained/ensemble/scoring targets and horizons, era partition, data/feature fingerprints, fit role, device, source run id, `selection_bias`, `split_estimand`) — no wall-clock or paths. `prediction_set_from_frame` / `read_prediction_set` require unique `(era, id)`, finite predictions, non-null keys, and sort `["era", "id"]`; submission stage additionally requires every value in `(0, 1)`. `compose_predictions` is the reusable composition entry: `Ensembler.blend` then optional `NeutralizationEngine.neutralize`, then optional per-era `tie_kept_rank` to `(0, 1)` for stage `submission`. `evaluate_prediction_set` forwards a `PredictionSet` to `evaluate_model` (capital stages `validation`/`submission`; `allow_research_stage=True` for research frames) and does not import `nmr.models`. `ExperimentRunner` still calls `Ensembler` / `NeutralizationEngine` directly so OOF bytes stay bit-identical.
