...
Validated prediction frames plus provenance so evaluation can consume a parquet that did not come from `nmr.models`. Closed stages: `PREDICTION_STAGES = ("raw", "blended", "neutralized", "validation", "submission")`. Closed partitions/roles: `ERA_PARTITIONS = ("oof", "validation", "live", "held_out")`, `FIT_ROLES = ("cv_oof", "full_history", "foreign")`. `PredictionProvenance` is frozen (stage, trained/ensemble/scoring targets and horizons, era partition, data/feature fingerprints, fit role, device, source run id, `selection_bias`, `split_estimand`, `validation_window`) — no wall-clock or paths; unknown closed-set values raise. `prediction_set_from_frame` / `read_prediction_set` require unique `(era, id)`, finite predictions, non-null keys, and sort `["era", "id"]`; submission stage additionally requires every value in `(0, 1)`. `compose_predictions` is the reusable composition entry: `Ensembler.blend` then optional `NeutralizationEngine.neutralize` (proportion must be finite in `[0, 1]`; missing feature columns may be inner-joined from an explicit `features` frame, and the joined keys must equal the prediction keys exactly), then optional per-era `tie_kept_rank` to `(0, 1)` for stage `submission`. Returned prediction columns are `Float64`. `as_submission` is a rank transform, not a capital promotion — OOF/held-out partitions are rejected. Capital `evaluate_prediction_set` requires a trusted `CapitalContext` (expected validation window, scoring target/horizon, data fingerprint, feature fingerprint) plus `stage="validation"`, `era_partition="validation"`, `selection_bias=False`, required scoring/fingerprint identity, and a locked `validation_window` matching both the frame and the context; missing identity is rejected. `submission` is not capital-eligible. Research stages return `ResearchEvaluation(is_capital=False)` only with `allow_research_stage=True`. Does not import `nmr.models`. `ExperimentRunner._run_validation_stage` scores through `evaluate_prediction_set`; OOF composition still calls `Ensembler` / `NeutralizationEngine` directly so OOF bytes stay bit-identical.
