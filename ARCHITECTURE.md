...
RunRegistry.promote_if_better() ─> champion.json  (guarded: scorecard metric + direction)
     |
     v
generate_dashboard.py (thin wrapper) ─> dashboard_ui.report ─> artifacts/dashboard.html  (executive report — offline single-file HTML)
...
| [generate_dashboard.py](generate_dashboard.py) | **Thin entry wrapper** — all logic in `dashboard_ui/report.py`; compiles the offline single-file `artifacts/dashboard.html` (executive report) on the `nmr/dashboard.py` engine; CLI surface unchanged (`python generate_dashboard.py`) |
| [dashboard_app.py](dashboard_app.py) | **Thin entry wrapper** — all logic in `dashboard_ui/app.py`; Streamlit+Plotly interactive dashboard over registry scorecards, benchmark CSV, campaign logs, and `fleet_summary` (§Q); read-only; launch: `streamlit run dashboard_app.py` |
| `dashboard_ui/charts.py` | Plotly figure builders + multimetric JS controller (data node + inlined `static/app.js`); consumes `nmr.dashboard` |
| `dashboard_ui/report.py` | HTML report compiler — `generate_dashboard` / `main`; inlines `static/style.css`; consumes `nmr.dashboard` + `dashboard_ui.charts` |
...
`nmr/dashboard.py` — executive report engine — unified leaderboard, tier-4 gate projection, stored-first capital recompute over the 86-era meta-overlap window, 7-metric multimetric timeseries (payout anchored to `target`; BMC self-guard; zeroed missing horizons), pairwise rank-gaussian similarity matrix with stress-regime delta; plotly/streamlit-free. `dashboard_ui/charts.py` adds the JS-controller multimetric chart (no `updatemenus`) and the similarity heatmap; `dashboard_ui/report.py` compiles the HTML (inlining `static/style.css`) and `dashboard_ui/app.py` renders the Streamlit views.
