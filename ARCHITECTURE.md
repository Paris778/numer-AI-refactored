...
All JSON writes: temp file in parent dir → fsync → `os.replace()`; the OOF parquet likewise writes temp + `os.replace` (no fsync). `record(result)` writes OOF then run.json; when `result.scorecard` is present, run.json carries a `scorecard` block of flat scalar keys per `MetricScorecard.to_frame()` (values filtered to drop `timing_*`/`quality_metric*` keys). **Registry quirk (do not trust `manifest.scoring_eras` on pre-rebuild rows):** runs recorded before the 2026-08-14 registry rebuild carry `scoring_eras` from the training-time validation window (e.g. `0461..0574`) while their `validation_preds.parquet` was regenerated on the refreshed window (e.g. `0583..1231`). The stored scorecard and the parquet agree; the manifest field is stale provenance. Never use `scoring_eras` to infer "what this run was scored on" — use the scorecard's `*_n_eras` cells and the parquet itself. `best(metric="sharpe")` validates the metric against `MetricSummary` fields (`mean`, `std`, `sharpe`, `max_drawdown`; unknown ⇒ `ValueError`) and returns the max with a run_id tiebreak (deterministic); `list()` sorts by (mtime, run_id) — stable. `promote(run_id)` regex-validates `[0-9a-f]{64}` (path-traversal guard), validates existence, then atomically rewrites champion.json. `promote_if_better(run_id, metric="corr_sharpe_ac") -> tuple[Path, bool]` promotes only when the candidate's scorecard metric strictly beats the champion's, honoring direction (`_SCORECARD_METRIC_DIRECTION`: `max_drawdown`/`std_corr` are lower-is-better); a scorecard-bearing candidate may displace a scorecard-less (legacy) champion; legacy candidates (no scorecard) and unknown metrics raise `ValueError`; a missing/corrupted champion pointer is treated as no champion.
...
| [generate_dashboard.py](generate_dashboard.py) | Executive performance report — `nmr/dashboard.py` engine (leaderboard, gate projection, capital recompute, payout timeseries) + `dashboard_charts.py` plotly layer; this script compiles the offline single-file `artifacts/dashboard.html` (primary), with `dashboard_app.py` as the thin Streamlit frontend on the same engine |
